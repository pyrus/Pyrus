...
foreach ($errs->E_WARNING as $error) {
    $test->assertEquals('warning: pear2.php.net/test conflicts with PHP extension "foo"', $error->getMessage(),
...
foreach ($errs->E_WARNING as $error) {
    $test->assertEquals('warning: pear2.php.net/test requires PHP extension "foo"', $error->getMessage(),
                        'foo fail message');
}
?>
===DONE===
