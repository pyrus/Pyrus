...
foreach ($errs->E_WARNING as $error) {
    $test->assertEquals('warning: pear2.php.net/test is not compatible with PHP extension "foo" version 2.0.0', $error->getMessage(),
                        'basic fail message');
}
...
foreach ($errs->E_WARNING as $error) {
    $test->assertEquals('warning: pear2.php.net/test conflicts with PHP extension "foo" (excluded versions: 2.0.0), installed version is 2.0.1', $error->getMessage(),
