--TEST--
PEAR2_Pyrus_Config::loadConfigFile() good userfile from saveConfig()
--FILE--
<?php
...
$a->openssl_cert = '/path/to/buh';
$a->http_proxy = 'hi';
$a->cache_ttl = 1;

$a->saveConfig();

copy($testpath . '/blah', $testpath . '/blah2');
$a = $configclass::singleton($testpath . '/foo', $testpath . '/blah2');

$test->assertEquals($testpath . DIRECTORY_SEPARATOR . 'foo', $a->path, 'peardir');
