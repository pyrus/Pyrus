...
class PEAR2_Pyrus
{
    ...
    static function download($url, $lastmodified = null, $accept = false, $doprogress = false)
    {
        ...
        if (PEAR2_Pyrus_Config::current()->http_proxy) {
            $request->proxy = PEAR2_Pyrus_Config::current()->http_proxy;
        ...
        $username = PEAR2_Pyrus_Config::current()->username;
        $password = PEAR2_Pyrus_Config::current()->password;
        ...
        if ($response->code >= 400) {
            if ($response->code == 404) {
                throw new PEAR2_Pyrus_HTTPException(
